...
february_checkbox.grid(row=3, column=0, sticky="ew")
...
monkey_simulation_button.grid(row=0, column=1, sticky="ew")

show_process_checkbox = ttk.Checkbutton(monkey_frame, text="Show process", variable=monkey.process)
show_process_checkbox.grid(row=0, column=0, sticky="ew")

monkey_typing_history = tk.Listbox(monkey_frame)
monkey_typing_history.grid(row=1, column=0, sticky="nsew", columnspan=2, rowspan=2)

a_percentage = 100 / pow(len(monkey.alphabet), len(monkey.word))
percentage_of_typing = tk.Label(text=f"{a_percentage}%")
percentage_of_typing.grid(row=2, column=0, sticky="ew")

set_new_word = tk.Entry()
set_new_word.grid(row=3, column=0, sticky="ew")
